...
def build_model(num_classes: int, dropout_p: float = 0.4):
    ...
    model.classifier[1] = nn.Conv2d(512, 156, kernel_size=1)
    return model
...
# ----------------------------
def main():
    parser = argparse.ArgumentParser(description="Fine-tune Squeezenet on custom dataset")
    parser.add_argument("--data_dir", type=str, default="dataset", help="Root dataset dir containing train/val/test")
    parser.add_argument("--out_dir", type=str, default="outputs/squeezenet1_0", help="Directory to save checkpoints and metrics")
