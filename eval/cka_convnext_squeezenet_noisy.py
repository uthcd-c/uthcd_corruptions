...
# Cell 3: Checkpoint helpers (force best.pt)
...
# Cell 4: Model factory (names align with common torchvision aliases)
...
def main():
    ...
    squeezenet = build_model("squeezenet1_0", num_classes=156, pretrained=True)
    meta = load_checkpoint_into_model(squeezenet, "outputs/squeezenet1_0/checkpoints/best.pt")
    
    loader, _, _, ds_mapping = make_test_loader(test_dir, batch_size=64, num_workers=8, invert=False,
                                                       add_noise=True,noise_std=30)
    cka = CKA(model1=convnext, model2=squeezenet,
    ...
    results = cka.export() 
    print(results)
    cka.plot_results(save_path="cka_convnext_squeezenet_noisy.png")
